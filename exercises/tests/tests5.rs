...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The function assumes that `address` points to a valid memory location
    // which is properly aligned for a `u32` value and doesn't violate any aliasing rules.
    // The function also assumes that modifying the value at this address won't cause
